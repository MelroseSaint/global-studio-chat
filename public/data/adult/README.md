...
`npm run data:sync` also mirrors every file into `public/data/adult/`, so
the deployed site serves them at
`https://purewire.vercel.app/data/adult/<file>`. Register that URL in the
Admin → Blocklist → External sources panel as a `domain`-format source,
then sync. Entries imported from a feed carry the feed's name as their
