...
export function Auth() {
  ...
  // Runtime probe of the auth stack's required env vars. Resolves to
  // { ok, missing } — `ok` false when the backend is misconfigured, with
  // `missing` naming the culprit variables (see serverIssue below).
  const authPreflight = useQuery(api.status.authPreflight);
  ...
  const authErrorMessage = (err: unknown, fallback: string): string => {
    ...
    if (/server error/i.test(msg)) {
      // A misconfigured backend explains itself here: the authPreflight
      // probe throws a ConvexError naming the missing env var, which lands
      // in `serverIssue`. Without it, show the honest per-flow fallback.
      if (serverIssue !== null) {
        return serverIssue;
      }
      if (step === "signup") {
