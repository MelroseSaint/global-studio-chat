 ...
 *   npm run qa:secrets
 *
 * A line may carry the marker `secrets-scan:ignore` to opt itself out —
 * used only for structural literals that merely LOOK like a secret shape
 * (e.g. the PEM armor strings ensure-jwt-keys.mjs rebuilds a canonical
 * key from; the armor is not key material). Every use must say WHY in an
 * adjacent comment, so the marker can't quietly hide a real leak.
 *
 * Exit codes: 0 clean, 1 secrets found.
 */
const IGNORE_MARKER = "secrets-scan:ignore";
...
function scanFile(file) {
  ...
  for (const [re, label] of PATTERNS) {
    for (let i = 0; i < lines.length; i++) {
      if (re.test(lines[i]) && !lines[i].includes(IGNORE_MARKER)) {
  ...
  for (const name of SECRET_ENV) {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (envRe.test(lines[i]) && !lines[i].includes(IGNORE_MARKER)) {
